<template>
  <div class="d-flex">
    <div style="width: calc(100% - 40px)">
      <SelectInput
        v-model="selectText"
        :options="Object.entries(user.decks)"
        :optionProjection="e => e[0]"
        :placeholder="user.selectedDeck || 'no deck'"
        @selectItem="
          e => {
            user.selectedDeck = e[0];
            selectText = '';
          }
        "
      ></SelectInput>
      ...
      <div class="d-flex justify-content-center">
        <div class="w-100">
          <div v-for="deck of Object.entries(user.decks)" :key="deck[0]" class="d-flex justify-content-between">
            ...
            <svg @click="deleteDeck(deck[0])" xmlns="http://www.w3.org/2000/svg" width="30" height="30" class="bi bi-trash" viewBox="0 0 16 16">
              <path
...
function deleteDeck(deckName: string) {
  ...
  if (user.value.selectedDeck == deckName) user.value.selectedDeck = Object.keys(user.value.decks)[0] || '';
}
</script>
<style scoped></style>
